...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($periode)) {
        $error = 'Rentang bulan wajib diisi!';
        ...
        if (!in_array($ext, ['xlsx', 'xls', 'csv'])) {
            ...
            try {
                ...
                if ($totalRows < 3) {
                    ...
                    if ($mode === 'replace') {
                        $stmtDel = $pdo->prepare("DELETE FROM ekspor_komoditas WHERE jenis_kegiatan = ? AND kategori = 'tumbuhan' AND periode = ?");
                    ...
                    if ($countCommodities > 0) {
                        $message = "Berhasil mengimpor <strong>{$countCommodities} komoditas tumbuhan</strong> untuk aktivitas <strong>{$labelKeg}</strong> (periode: <strong>" . htmlspecialchars($periode) . "</strong>)!";
                    } else {
...
<head>
    ...
    <title>Upload Excel Komoditas Tumbuhan (KT) - Admin Karantina</title>
...
<body>
    ...
    <div class="admin-main" style="max-width: 860px;">
        ...
        <div class="page-header">
            <div>
                <span class="eyebrow" style="background: var(--cat-tumbuhan-soft); color: var(--cat-tumbuhan); border-color: rgba(21,128,61,0.2);"><span class="dot dot-tumbuhan"></span> Karantina Tumbuhan (KT)</span>
                <h1 class="page-title">Upload Excel Komoditas Tumbuhan</h1>
            </div>
            <a href="template_sample.php?kategori=tumbuhan" class="btn btn-secondary">
                ...
                <span>Download Template Tumbuhan</span>
            </a>
        </div>
        ...
        <?php if (!empty($message)): ?>
            <div class="alert alert-success" style="margin-bottom: 16px; justify-content: space-between; flex-wrap: wrap;">
                ...
                <a href="kelola_data.php?jenis=<?= $currentJenis ?>&kategori=tumbuhan" class="btn btn-primary btn-sm">Lanjut Kelola & Publish Data &rarr;</a>
            </div>
        <?php endif; ?>
            ...
            <form action="" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 22px;">
                ...
                <div>
                    ...
                    <div class="tab-row grid-3">
                        ...
                        <a href="upload_tumbuhan.php?jenis=<?= $currentJenis ?>" class="tab-link active kategori-tab-link"><span class="dot dot-tumbuhan"></span> Tumbuhan (KT)</a>
                    </div>
                </div>
                ...
                <div class="card card-pad" style="background: var(--bg);">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px;">
                        <div>
                            ...
                            <input type="month" name="bln_awal" value="<?= date('Y-01') ?>" class="field-input">
                        </div>
                        <div>
                            <label class="field-label">Sampai Bulan</label>
                            <input type="month" name="bln_akhir" value="<?= date('Y-12') ?>" class="field-input">
                        </div>
                    </div>

                    <input type="hidden" name="cal_mode" value="bulan">

                    <p class="field-hint">Data yang diunggah akan otomatis memiliki satu periode dan tanggal mulai/selesai sesuai bulan yang dipilih.</p>
                </div>
                ...
                <div style="padding-top: 4px; display: flex; align-items: center; justify-content: space-between;">
                    <a href="kelola_data.php?jenis=<?= $currentJenis ?>&kategori=tumbuhan" style="font-size: 13px; color: var(--ink-muted); text-decoration: none;">Batal / Ke Kelola Data</a>
